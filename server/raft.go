...
// waits for any fast path proposals that are in progress to complete.
// Lock should be held.
...
func (n *raft) maxBatchSize() int {
	...
	if opts := n.s.getOpts(); opts.MaxPayload > 0 {
		maxPayload = int(opts.MaxPayload)
	}
	if av, ok := n.s.accounts.Load(n.accName); ok {
		acc := av.(*Account)
		acc.mu.RLock()
		if acc.mpay > 0 {
